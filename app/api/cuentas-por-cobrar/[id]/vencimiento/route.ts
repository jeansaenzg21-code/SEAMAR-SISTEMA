...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [rows]: any = await pool.query(
      `SELECT id, vencimiento_origen, numero_factura FROM cuentas_por_cobrar WHERE id = ? LIMIT 1`,
      [Number(id)]
    );
    ...
    await pool.query(
      `UPDATE cuentas_por_cobrar SET fecha_vencimiento = ?, vencimiento_origen = 'MANUAL' WHERE id = ?`,
      [fecha, Number(id)]
    );
    ...
    registrarActividad({
      tipo: "cxc",
      accion: "actualizar",
      titulo: `Vencimiento actualizado: ${rows[0].numero_factura || "CxC #" + id}`,
    ...
    console.error("Error al actualizar vencimiento (CxC):", error);

...
function getSessionStatusConfig(status: SessionStatus) {
  const configs = {
    ...
    PENDIENTE:       { label: 'Pendiente',       bg: 'bg-slate-700',      text: 'text-slate-300'   },
    CON_DIFERENCIAS: { label: 'Con diferencias', bg: 'bg-orange-500/10',  text: 'text-orange-400'  },
  }
  return configs[status]
}
...
function KPICard({
  ...
  return (
    <div className="bg-slate-900 rounded-lg border border-slate-700/50 p-4 flex flex-col gap-3">
...
export default function InvoiceReconciliationContent({
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/conciliacion-facturas", {
        method: "POST",
        body: formData
      })
      const data = await response.json()
      const nuevosRegistros = [
        ...(data.coincidenciasDetalle || []).map((r: any, index: number) => ({
  id: `C-${index}`,
  ...
  observation: null
        ...
        ...(data.noEncontradasDetalle || []).map((r: any, index: number) => ({
          id: `N-${index}`,
          invoiceNumber: r.factura,
          ...
          observation: "No existe en cuentas_por_pagar"
        })),
        ...(data.diferenciasMontoDetalle || []).map((r: any, index: number) => ({
          id: `D-${index}`,
          invoiceNumber: r.factura,
...
date:
  r.fecha || "",
      ...
      setKpiData({
        totalExcel: data.totalExcel,
        matches: data.coincidencias,
        observations: data.noEncontradas,
        amountDifferences: data.diferenciasMonto
      })
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans">
      ...
      <div className="bg-slate-950 border-b border-slate-800 px-6 py-4">
        <div className="max-w-[1600px] mx-auto">
          <div className="flex items-center gap-2 text-xs text-slate-600 mb-1">
            ...
            <span>Tesorería</span>
            <span>/</span>
            <span className="text-slate-400 font-medium">Conciliación de Facturas</span>
          </div>
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-600 bg-slate-900 border border-slate-700/50 rounded-md px-3 py-1.5 shrink-0">
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-5 space-y-5">
        ...
        {/* ── Upload Panel ── */}
        <div className="bg-slate-900 rounded-lg border border-slate-700/50 p-5">
          <div className="flex items-center gap-2 mb-4">
            <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" />
            </svg>
            <h2 className="text-sm font-semibold text-slate-300">Seleccionar archivo Excel</h2>
          </div>
          ...
          <div className="flex flex-col xl:flex-row gap-4">
            ...
            <div
              ...
              className={`flex-1 border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors ${
                uploadedFile
                  ...
                  : 'border-slate-700 bg-slate-800/40 hover:border-blue-500/50 hover:bg-blue-500/5'
              }`}
              ...
              {uploadedFile ? (
                <>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-emerald-600 mt-0.5">{fileSizeLabel}</p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-slate-600 mt-0.5">o haz clic para explorar — .xlsx, .xls</p>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 xl:w-64">
              {uploadedFile && (
                <div className="bg-slate-800 rounded-md border border-slate-700/50 p-3 space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">Archivo</span>
                    <span className="text-slate-200 font-medium truncate max-w-[140px]">{uploadedFile.name}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">Tamaño</span>
                    <span className="text-slate-200">{fileSizeLabel}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">Cargado</span>
                    <span className="text-slate-200">{new Date().toLocaleDateString('es-PE')}</span>
                  </div>
                </div>
              )}
              <button
                ...
                className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-600 disabled:border disabled:border-slate-700/50 text-white text-sm font-medium rounded-md transition-colors flex items-center justify-center gap-2"
              >
                {isProcessing ? (
                  <>
              ...
              {uploadedFile && (
                <button
                  ...
                  className="w-full py-2 px-4 border border-slate-700/50 text-slate-400 text-sm rounded-md hover:bg-slate-800 hover:text-slate-300 transition-colors"
                >
                  Limpiar
                </button>
              )}
        ...
        <div className="bg-slate-900 border border-slate-700/50 rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    {tab.count !== undefined && (
                      <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${
                        isActive ? 'bg-blue-500/15 text-blue-400' : 'bg-slate-800 text-slate-500'
                      }`}>
                        {tab.count}
                      </span>
                    )}
            ...
            <div>
              {sessions.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                  ...
                  <p className="text-xs mt-1 text-slate-600">Las conciliaciones procesadas aparecerán aquí</p>
                </div>
              ) : (
                  ...
                  {sessions.map((session) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-slate-800/60 transition-colors ${
                          isActive ? 'bg-blue-500/5 border-l-2 border-blue-500' : ''
                        }`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-slate-200 truncate">{session.fileName}</p>
                          <div className="flex items-center gap-3 mt-0.5">
                            <span className="text-xs text-slate-600">{formatDate(session.date)}</span>
                            <span className="text-xs text-emerald-500">{session.matches} coincidencias</span>
                            {session.observations > 0 && (
                              <span className="text-xs text-orange-500">{session.observations} obs.</span>
                            )}
                          </div>
                        </div>
          ...
          {activeTab === 'resultados' && (
            <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  {selectedSession && (
                    <>
                      ...
                      <span className="font-medium text-slate-400 truncate max-w-[200px]">{selectedSession.fileName}</span>
                    </>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-8 pr-3 py-1.5 text-xs border border-slate-700/50 bg-slate-800 text-slate-300 placeholder-slate-600 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500/50 w-52"
                    />
                  </div>
                  <select
                    value={filterStatus}
                    onChange={(e) => setFilterStatus(e.target.value as ReconciliationStatus | 'ALL')}
                    className="text-xs border border-slate-700/50 bg-slate-800 text-slate-300 rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                  >
              ...
              {records.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                  <svg className="w-12 h-12 mb-3 text-slate-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                    <polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" />
                  </svg>
                  <p className="text-sm font-medium text-slate-500">Sin resultados aún</p>
                  <p className="text-xs mt-1 max-w-xs text-slate-600">
                    Carga un archivo Excel y presiona{' '}
                    <span className="font-medium text-slate-400">Procesar Conciliación</span>{' '}
                ...
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="text-xs mt-1 text-slate-600">Ajusta los filtros o el término de búsqueda</p>
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {filteredRecords.map((record) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-2.5 font-mono font-medium text-slate-200 whitespace-nowrap">
                            ...
                            <td className="px-4 py-2.5 tabular-nums whitespace-nowrap">
                              {record.amountSystem !== null ? (
                                <span className={record.amountSystem !== record.amountExcel ? 'text-red-400 font-medium' : 'text-slate-300'}>
            ...
            <div>
              {!selectedRecord ? (
                <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
                  ...
                  <p className="text-xs mt-1 text-slate-600">Selecciona una factura desde Resultados para ver el detalle</p>
                  <button
                    onClick={() => setActiveTab('resultados')}
                    className="mt-4 text-xs text-blue-400 hover:text-blue-300 hover:underline font-medium transition-colors"
                  >
                    ← Ir a Resultados
                  </button>
                </div>
                ...
                <div className="max-w-2xl mx-auto">
                  ...
                  <div className="flex items-center gap-2 px-6 pt-4 pb-2 text-xs text-slate-600">
                    <button
                    ...
                    <span>›</span>
                    <span className="text-slate-300 font-medium">{selectedRecord.invoiceNumber}</span>
                  </div>
                  ...
                  <div className={`mx-6 rounded-lg border flex items-center gap-3 px-4 py-3 mb-5 ${getStatusConfig(selectedRecord.status).bg} ${getStatusConfig(selectedRecord.status).border}`}>
                    ...
                    <span className="text-xs text-slate-600 ml-auto">{formatDate(selectedRecord.date)}</span>
                  </div>

                  <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Datos de factura */}
                    <section className="bg-slate-950 border border-slate-800 rounded-lg p-4 space-y-3">
                      <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest">Factura</p>
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500">Número</span>
                          <span className="font-mono font-semibold text-slate-100">{selectedRecord.invoiceNumber}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500">Fecha</span>
                          <span className="text-slate-300">{formatDate(selectedRecord.date)}</span>
                        </div>
                      </div>
                    </section>
                    ...
                    {/* Datos de proveedor */}
                    <section className="bg-slate-950 border border-slate-800 rounded-lg p-4 space-y-3">
                      <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest">Proveedor</p>
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs gap-2">
                          <span className="text-slate-500 shrink-0">Razón social</span>
                          <span className="text-slate-300 text-right">{selectedRecord.supplier}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-slate-500">RUC</span>
                          <span className="font-mono text-slate-300">{selectedRecord.ruc}</span>
                        </div>
                      </div>
                    </section>

                    {/* Comparación de montos — ancho completo */}
                    <section className="md:col-span-2 bg-slate-950 border border-slate-800 rounded-lg p-4 space-y-3">
                      <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest">Comparación de Montos</p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-slate-900 rounded-md p-3 text-center border border-slate-700/50">
                          <p className="text-[10px] text-slate-500 mb-1">Excel</p>
                        ...
                        >
                          <p className="text-[10px] text-slate-500 mb-1">Sistema</p>
                          <p
                            className={`text-base font-semibold tabular-nums ${
                              selectedRecord.amountSystem === null
                                ? 'text-orange-400 italic text-xs'
                                : selectedRecord.amountSystem !== selectedRecord.amountExcel
                                ? 'text-red-400'
                                : 'text-emerald-400'
                            }`}
                      ...
                      {selectedRecord.amountSystem !== null &&
                        selectedRecord.amountSystem !== selectedRecord.amountExcel && (
                          <div className="flex justify-between text-xs bg-red-500/5 rounded px-3 py-2 border border-red-500/20">
                    ...
                    <section className="md:col-span-2 bg-slate-950 border border-slate-800 rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        <p className="text-[10px] font-semibold text-slate-600 uppercase tracking-widest">Observación</p>
                        {!editingObservation && (
                          <button
                      ...
                      {editingObservation ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            rows={3}
                            className="w-full text-xs border border-slate-700/50 bg-slate-900 text-slate-300 placeholder-slate-600 rounded-md px-2.5 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500/50 resize-none"
                            placeholder="Escribe la observación…"
                          />
                          <div className="flex gap-2">
                            <button
                              onClick={handleSaveObservation}
                              className="flex-1 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded-md transition-colors"
                            >
                              Guardar
                            </button>
                            <button
                              onClick={() => setEditingObservation(false)}
                              className="flex-1 py-1.5 border border-slate-700/50 text-slate-400 text-xs rounded-md hover:bg-slate-800 transition-colors"
                            >
                              Cancelar
                            </button>
                          </div>
                        ...
                        <p className="text-xs text-slate-400 leading-relaxed bg-slate-900 rounded-md px-2.5 py-2 border border-slate-800">
                          {selectedRecord.observation}
                        </p>
                      ) : (
                        <p className="text-xs text-slate-700 italic">Sin observaciones</p>
                      )}
                    </section>
                  </div>

                  {/* Action buttons */}
                  <div className="px-6 pb-6 flex flex-wrap gap-3 border-t border-slate-800 pt-5">
                    <button
                      onClick={() => setEditingObservation(true)}
                      className="flex items-center gap-1.5 py-2 px-4 text-xs font-medium border border-slate-700/50 text-slate-400 rounded-md hover:bg-slate-800 hover:text-slate-300 transition-colors"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 py-2 px-4 text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white rounded-md transition-colors ml-auto"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <polyline points="20 6 9 17 4 12" />
                      </svg>
